...
class VideoThread(QThread):
    ...
    def capture_dataset(self):
        ...
        if not self.person_name:
            self.update_status.emit("Error: No person name provided")
            self.capture_complete.emit(False)
            return
            
        # Create directory for this person if it doesn't exist
        dataset_dir = self.face_system.db_manager.dataset_dir
        if not dataset_dir:
            self.update_status.emit("Error: Dataset directory not configured")
            self.capture_complete.emit(False)
            return
            
        person_dir = Path(dataset_dir) / str(self.person_name)
        ...
        try:
            ...
            while self.running and count < 500:
                ...
                for (top, right, bottom, left) in scaled_face_locations:
                    ...
                    img_name = str(person_dir / f"{self.person_name}_{count}.jpg")
